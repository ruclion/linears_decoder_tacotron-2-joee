...
def run_eval(args, checkpoint_path, output_dir, hparams, sentences, speaker_id_lists):
	...
	batch_speaker_id_lists = [speaker_id_lists[i: i+hparams.tacotron_synthesis_batch_size] for i in range(0, len(speaker_id_lists), delta_size)]
	print('here is what????????', delta_size)
	print(batch_sentences)
	print(batch_speaker_id_lists)
	start = time.time()
	for i, batch in enumerate(tqdm(batch_sentences)):
		audio.save_wav(synth.eval(batch, batch_speaker_id_lists[i]), os.path.join(log_dir, 'wavs', 'eval_batch_{:03}.wav'.format(i)), hparams)
